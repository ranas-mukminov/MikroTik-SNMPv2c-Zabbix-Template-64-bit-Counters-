...
class CostEstimator:
    def __init__(self, price_per_kwh: float, currency: str = "USD", hours_per_month: int = 730) -> None:
    ...
    def estimate(self, power_by_node: Dict[str, float]) -> Dict[str, Dict[str, float]]:
        ...
        for node, watts in power_by_node.items():
            ...
            result[node] = {"kwh": round(kwh, 2), "monthly_cost": round(cost, 2), "currency": self.currency}
